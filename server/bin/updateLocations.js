...
const main = async () => {
	...
	let user = await usersCollection.find({username:username})
	user = user[0]
	let city = await locationsCollection.find({location:user.city})	
	city = city[0]
	const top = await topLanguagesAgg(db, user.city, 1)
	await locationsCollection.update({location:user.city}, 
		{$set:{topLanguage: top[0].language, users:city.users+1, repos: city.repos+user.publicRepos }});
	db.close()
}
main()